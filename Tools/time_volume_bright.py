...
async def control_system_volume(prompt: str, volume_level: int) -> str:
    """
    Adjusts system volume using native Windows COM interface.
    """
    try:
        ...
        ps_script = os.path.join(os.path.dirname(__file__), "volume_control.ps1")
        if os.path.exists(ps_script):
            subprocess.run(
                ["powershell", "-ExecutionPolicy", "Bypass", "-File", ps_script, "-action", "set", "-value", str(volume_level)],
                capture_output=True, text=True, timeout=5
            )
            return f"✅ System volume has been exactly set to {volume_level}%."
        else:
            return "❌ Missing volume_control.ps1 script."
...
async def get_current_volume() -> str:
    """
    Get current system volume level using native Windows COM interface.
    ...
    try:
        ...
        if os.path.exists(ps_script):
            result = subprocess.run(
                ["powershell", "-ExecutionPolicy", "Bypass", "-File", ps_script, "-action", "get"],
                capture_output=True, text=True, timeout=5
            )
            vol = result.stdout.strip()
            return f"🔊 Current System Volume: {vol}%" if vol else "❌ Could not determine volume."
        else:
            return "❌ Missing volume_control.ps1 script."
