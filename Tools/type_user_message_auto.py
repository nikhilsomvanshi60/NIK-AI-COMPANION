...
@function_tool()
async def modify_essay_in_notepad(change_request: str) -> str:
    """
    Modifies the currently open essay in Notepad based on change request.
    Selects all text, deletes it, and writes modified version with changes.

    Args:
        change_request: Description of changes to make (e.g., "add more points about benefits", "make it more formal", "shorten to 200 words")
        
    Returns:
        str: Success confirmation with modification details
    """
    
    if not change_request.strip():
        return "❌ कृपया बताएं क्या बदलाव करने हैं | Please specify what changes to make"
    
    try:
        print(f"🔄 ESSAY MODIFICATION REQUEST: '{change_request}'")
        
        # Store original position
        original_pos = pyautogui.position()
        
        # STEP 1: Make sure Notepad is active and get current content
        print("📋 PHASE 1: Reading Current Essay...")
        
        # Click on Notepad to ensure focus
        await asyncio.to_thread(pyautogui.click)
        await asyncio.sleep(0.3)
        
        # Select all and copy to get current content
        await asyncio.to_thread(pyautogui.hotkey, 'ctrl', 'a')
        await asyncio.sleep(0.3)
        await asyncio.to_thread(pyautogui.hotkey, 'ctrl', 'c')
        await asyncio.sleep(0.3)
        
        # Get current content from clipboard
        try:
            import pyperclip
            current_content = pyperclip.paste()
        except ImportError:
            return "❌ pyperclip required for essay modification"
        
        if not current_content.strip():
            return "❌ No content found in Notepad. Please open an essay first."
        
        print(f"✅ Current essay read: {len(current_content.split())} words")
        
        # Extract topic from current essay
        lines = current_content.split('\n')
        current_topic = "Essay"
        for line in lines:
            if line.startswith("ESSAY:"):
                current_topic = line.replace("ESSAY:", "").strip()
                break
        
        print(f"📖 Topic: {current_topic}")
        
        # STEP 2: Generate modified content using Groq
        print("🤖 PHASE 2: Generating Modified Content...")
        
        GROQ_API_URL = "https://api.groq.com/openai/v1/chat/completions"
        GROQ_API_KEY = os.getenv("GROQ_API_KEY", "")
        
        system_prompt = """You are a professional essay writer and editor. Modify the provided essay according to the change request.

IMPORTANT RULES:
1. Return the COMPLETE modified essay with all sections
2. Preserve the essay title/heading format (ESSAY: [Topic])
3. Preserve the date line if present
4. Apply the requested changes while maintaining essay quality
5. Keep similar word count unless asked to change length
6. Maintain proper paragraph structure
7. Add/modify word count at the end

Return the complete modified essay with same formatting as original."""

        payload = {
            "model": "openai/gpt-oss-120b",
            "messages": [
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": f"CURRENT ESSAY:\n{current_content}\n\nCHANGE REQUEST: {change_request}\n\nProvide the complete modified essay."}
            ],
            "temperature": 0.4,
            "max_tokens": 4096
        }
        ...
        print(f"✅ Modified content generated: {new_word_count} words")
        
        # STEP 3: Replace content in Notepad
        print("⌨️ PHASE 3: Replacing Essay Content...")
        ...
        lines = modified_content.split('\n')
        for i, line in enumerate(lines):
            if line.strip() or i < len(lines) - 1:  # Keep empty lines for formatting
                await asyncio.to_thread(pyautogui.typewrite, line, interval=0.02)
            if i < len(lines) - 1:
                await asyncio.to_thread(pyautogui.press, 'enter')
                await asyncio.sleep(0.05)
        
        # STEP 4: Save modified essay
        print("💾 PHASE 4: Saving Changes...")
        await asyncio.sleep(0.5)
        await asyncio.to_thread(pyautogui.hotkey, 'ctrl', 's')
        ...
        # Prepare response in Hindi/English
        hindi_response = f"""✅ निबंध में बदलाव सफल! | Essay Modified Successfully!

📖 विषय/Topic: {current_topic}
📝 बदलाव/Changes: {change_request}
📊 पुराने शब्द/Old Words: {len(current_content.split())}
📊 नए शब्द/New Words: {new_word_count}
🎯 स्थिति/Status: बदलाव सफल | Modification Complete

बदलाव लागू कर दिए गए हैं! | Changes have been applied!"""
        
        print(f"✅ Essay modified: {new_word_count} words")
...
@function_tool()
async def improve_essay_style(style_request: str) -> str:
    """
    Improves the essay style in Notepad (formal, academic, simple, persuasive, etc.)
    
    Args:
        style_request: Desired style (e.g., "make it more formal", "academic style", "simple language")
        
    Returns:
        str: Success confirmation
    """
    return await modify_essay_in_notepad(f"Rewrite the essay in {style_request} style while maintaining all key points")


@function_tool()
async def add_points_to_essay(points: str) -> str:
    """
    Adds specific points or arguments to the current essay
    
    Args:
        points: Additional points to include
        
    Returns:
        str: Success confirmation
    """
    return await modify_essay_in_notepad(f"Add these points to the essay: {points}. Integrate them naturally into the existing content.")